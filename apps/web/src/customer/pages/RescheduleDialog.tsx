...
export function RescheduleDialog({ booking, onDone }: { booking: Booking; onDone?: () => void }) {
  ...
  async function onFinish(values: RescheduleFormValues) {
    ...
    try {
      ...
      if (status === 409) {
        message.error(t("customer:rescheduleDialog.alreadyPending"));
      } else {
        message.error(t("customer:rescheduleDialog.submitError"));
      }
    }
  }
          ...
          >
            <Select
              ...
              options={(slots ?? []).map((slot) => ({
                value: slot.id,
                label: `${formatDateTime(slot.date, i18n.language)} ${slot.startTime}–${slot.endTime}`,
